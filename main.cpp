...
constexpr unsigned int HARD_MINIMUM_LENGTH = 0;
constexpr unsigned int SOFT_MAXIMUM_LENGTH = 100;
constexpr unsigned int HARD_MAXIMUM_LENGTH = 150;
constexpr unsigned int SOFT_TRY = 20;

// Memory usage
constexpr unsigned int MINIMUM_OCCURENCES = 2;
constexpr unsigned int MINIMUM_SCORE = 3;
constexpr unsigned int REHASH_THRESOLD = 150000;
...
void clean(string& s, string& username) {
  static auto isnotspace = [](char c) { return !isspace(c); };

//sterg usernameul din intrari
  eraseAllSubStr(s, username);
...
int main(int argc ,char **argv) {
  ...
  Bot bot(argv[1]);
  //std::cout << "s-a creat bot cu nickname: " << argv[1] << endl;
  ...
  while (true) {
    ...
    bool nume_aparut = search(body.begin(), body.end(), bot.nickname.begin(), bot.nickname.end(), chieq) != body.end();
    clean(body, bot.nickname);
    ...
    if (username == "###") {
      ...
      //std::cout << "invat " << body << std::endl;
      bot.learn(body);

      if (bot.enabled && nume_aparut) {
